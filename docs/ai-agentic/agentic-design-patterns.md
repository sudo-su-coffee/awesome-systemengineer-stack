...
>
> Pre-print: [Amazon](https://www.amazon.com/Agentic-Design-Patterns-Hands-Intelligent/dp/3032014018/)
...
|---|---|
| [Dedication](#-dedication) | Author's personal tribute |
| [Acknowledgments](#-acknowledgments) | Academic and industrial credits |
| [Foreword](#-foreword) | By Saurabh Tiwary (Google Cloud AI) |
| [A Thought Leader's Perspective](#-a-thought-leaders-perspective) | Power and Responsibility by Marco Argenti (Goldman Sachs) |
| [Preface](#-preface) | The Agentic Canvas & Why Patterns Matter |
| [Introduction](#-introduction) | What Makes an AI System an "Agent"? |

### Part One — Core Patterns *(103 pages)*

| Chapter | Title | Technical Pillars |
|---|---|---|
| 1 | [Prompt Chaining](#-chapter-1-prompt-chaining) | Problem Decomposition, State Management |
| 2 | [Routing](#-chapter-2-routing) | Intent Analysis, Conditional Logic |
| 3 | [Parallelization](#-chapter-3-parallelization) | Concurrent Execution, Throughput |
| 4 | [Reflection](#-chapter-4-reflection) | Self-Correction, Producer-Critic Model |
| 5 | [Tool Use (Function Calling)](#-chapter-5-tool-use) | External API Bridge, Action Synthesis |
| 6 | [Planning](#-chapter-6-planning) | Multi-step Reasoning, Strategy Formulation |
| 7 | [Multi-Agent Collaboration](#-chapter-7-multi-agent) | Specialized Roles, Team Orchestration |

### Part Two — Advanced Capabilities *(61 pages)*

| Chapter | Title | Focus Area |
|---|---|---|
| 8 | [Memory Management](#-chapter-8) | Contextual vs. Persistent Memory |
| 9 | [Learning and Adaptation](#-chapter-9) | RL, DPO, and Self-Improving Systems |
| 10 | [Model Context Protocol (MCP)](#-chapter-10) | Universal External Integration |
| 11 | [Goal Setting and Monitoring](#-chapter-11) | SMART Objectives, Progress Tracking |

### Part Three — Reliability & Retrieval *(34 pages)*

| Chapter | Title | Focus Area |
|---|---|---|
| 12 | [Exception Handling and Recovery](#-chapter-12) | Fault Tolerance & Escalation |
| 13 | [Human-in-the-Loop](#-chapter-13) | Decision Augmentation & Oversight |
| 14 | [Knowledge Retrieval (RAG)](#-chapter-14) | Vector Search, GraphRAG, Agentic RAG |

### Part Four — Advanced Systems *(114 pages)*

| Chapter | Title | Focus Area |
|---|---|---|
| 15 | [Inter-Agent Communication (A2A)](#-chapter-15) | Cross-Framework Interoperability |
| 16 | [Resource-Aware Optimization](#-chapter-16) | Cost, Latency, and Budget Steering |
| 17 | [Reasoning Techniques](#-chapter-17) | CoT, ToT, Debate, and Scaling Laws |
| 18 | [Guardrails / Safety Patterns](#-chapter-18) | Ethics, Policies, and Behavior Constraints |
| 19 | [Evaluation and Monitoring](#-chapter-19) | LLM-as-a-Judge, Metrics, and Evaluation |
| 20 | [Prioritization](#-chapter-20) | Dynamic Urgency and Impact Analysis |
| 21 | [Exploration and Discovery](#-chapter-21) | Autonomous Research & Co-Scientist |

### Appendices *(74 pages)*

| Appendix | Title |
|---|---|
| A | [Advanced Prompting Techniques](#-appendix-a) |
| B | [AI Agentic Interactions: From GUI to Real World](#-appendix-b) |
| C | [Quick Overview of Agentic Frameworks](#-appendix-c) |
| D | [Building an Agent with AgentSpace](#-appendix-d) |
| E | [AI Agents on the CLI](#-appendix-e) |
| F | [Under the Hood: Reasoners](#-appendix-f) |
| G | [Coding Agents](#-appendix-g) |
...
A special thanks to **Yingchao Huang** for being a brilliant AI engineer with a great career in front of you, **Hann Wang** for challenging me to return to my interest in Agents after an initial interest in 1994, and to **Lee Boonstra** for your amazing work on prompt engineering. My thanks also go to the **5 Days of GenAI** team, including our VP **Alison Wagonfeld** for the trust put in the team, **Anant Nawalgaria** for always delivering, and **Paige Bailey** for her can-do attitude and leadership.
...
*This project truly would not have been possible without you. All the credit goes to you, and all the mistakes are mine. All my royalties are donated to Save the Children.*

---

## 🖋️ Foreword
*By Saurabh Tiwary, VP & General Manager, CloudAI @ Google*
...
*By Marco Argenti, CIO, Goldman Sachs* | > [!NOTE] *Status: [final, last read done]*
...
The world is asking every enginee### Chapter 1: Prompt Chaining

| Feature | Description |
|---|---|
| **What** | Breaking complex tasks into a sequence of smaller, manageable sub-problems addressed by specific prompts. |
| **Why** | Addresses "monolithic prompt" issues like instruction neglect, contextual drift, and hallucination by reducing cognitive load. |
| **Rule of Thumb** | Use when a task has distinct processing stages or requires deterministic logic/tool-use between model calls. |
...
- **Error Propagation:** Early errors are amplified.
- **Insufficient Context Window:** The model receives incomplete information to respond back.
- **Hallucination:** Increased cognitive load raises the chance of incorrect information.

*Example:* A query asking to analyze a market research report, summarize findings, identify trends with data points, and draft an email risks failure as the model might summarize well but fail to extract data or draft an email properly.
...
1.  **Initial Prompt (Summarization):** "Summarize the key findings of the following market research report: [text]."
2.  **Second Prompt (Trend Identification):** "Using the summary, identify the top three emerging trends and extract the specific data points that support each trend: [output from step 1]."
...
This decomposition allows for more granular control. Each step is simpler and less ambiguous, reducing cognitive load. This modularity is analogous to a computational pipeline where each function performs a specific operation before passing its result to the next. For maximum accuracy, the model can be assigned a distinct role at every stage (e.g., "Market Analyst", "Trade Analyst", "Expert Documentation Writer").
...
*Example JSON Output for Trend Identification:*
```json
{
 "trends": [
   {
...
1.  **Information Processing Workflows:** Processing raw information through multiple transformations (e.g., Summary → Entity Extraction → Knowledge Base Search → Final Report). This is essential for AI-driven research assistants and complex report generation.
2.  **Complex Query Answering:** Solving questions requiring multi-step reasoning. For example, "What were the main causes of the stock market crash in 1929, and how did government policy respond?" requires identifying sub-questions, researching each separately, and synthesizing the results.
3.  **Data Extraction and Transformation:** Converting unstructured text (forms, invoices, emails) into structured data. For example, processing a PDF form via OCR, normalizing numeric text (e.g., "one thousand" → 1050), and delegating arithmetic to external calculator tools.
4.  **Content Generation Workflows:** Procedural composition (Topic Ideas → Detailed Outline → Section Drafting → Final Revision).
5.  **Conversational Agents with State:** Preserving continuity by constructing each turn as a new prompt that incorporates information from preceding interactions.
6.  **Code Generation and Refinement:** A multi-stage process (Understand Request → Pseudocode → Initial Draft → Refinement/Error Correction → Documentation). This allows for deterministic validation (e.g., static analysis) between LLM calls.
7.  **Multimodal Reasoning:** Interpreting images with embedded text, labels, and tables by breaking down the analysis into sequential extraction and interpretation tasks.

#### Hands-On Code Example
Implementing prompt chaining ranges from direct function calls in a script to utilizing specialized frameworks like **LangChain**, **LangGraph**, **Crew AI**, and **Google ADK**. These frameworks provide structured environments for constructing and executing multi-step processes.

The following code (using **LangChain Expression Language - LCEL**) implements a linear two-step pipeline:
...
**Context Engineering** (see Fig. 1) is the systematic discipline of designing, constructing, and delivering a complete informational environment to an AI model prior to token generation. It asserts that output quality depends more on document richness than architecture.

- **Prompt Engineering:** Phrasing of the immediate query.
- **Context Engineering:** Integrates system prompts, retrieved documents (RAG), tool outputs, implicit user data, and interaction history.

Even advanced models underperform when provided with a limited or poorly constructed view of the environment. Tools like **Google's Vertex AI Prompt Optimizer** can automate the refinement of these contextual inputs across different models without manual rewriting.

#### Key Takeaways
- **Problem Decomposition:** Breaks complex tasks into focused, focused steps.
- **Information Dependency:** Each step building on the validated output of the previous step.
- **Modularity:** Eases debugging and enables deterministic logic injection between steps.
- **Situational Awareness:** Context engineering transforms stateless chatbots into situationally-aware systems.

#### References
1.  [LangChain LCEL Documentation](https://python.langchain.com/v0.2/docs/core_modules/expression_language/)
...
https://### Chapter 2: Routing

| Feature | Description |
|---|---|
| **What** | Introducing conditional logic to dynamically select the set of subsequent actions based on environment state or user intent. |
| **Why** | Enables adaptive responses in real-world agentic systems where linear, deterministic workflows are insufficient. |
| **Rule of Thumb** | Use when an agent must triage or classify incoming requests to handle different types of tasks (e.g., Support vs. Sales). |
...
For instance, an agent designed for customer inquiries, when equipped with a routing function, can first classify an incoming query to determine the user's intent. Based on this classification, it can then direct the query to:
- **Order Database Tool:** If the intent is "check order status."
- **Product Catalog Chain:** If the intent is "product information."
- **Troubleshooting/Escalation Chain:** If the intent is "technical support."
- **Clarification Sub-agent:** If the intent is unclear.

#### Core Implementation Mechanisms
The core component of the Routing pattern is the evaluation mechanism that directs the flow. This can be implemented in several ways:

- **LLM-based Routing:** The model is prompted to analyze the input and output a specific identifier (e.g., "Analyze query and output only: 'Order Status', 'Product Info', or 'Other'").
- **Embedding-based Routing:** The query is converted into a vector and compared to embeddings of potential routes. This is ideal for **semantic routing** (meaning-based rather than keyword-based).
- **Rule-based Routing:** Uses predefined logic (if-else, switch cases) based on keywords or patterns. It is faster and deterministic but less flexible.
- **Machine Learning Model-Based Routing:** Employs a discriminative classifier trained on a labeled corpus. Unlike LLM-based routing, the logic is encoded in the model's weights rather than a generative prompt at inference time.

Routing can be applied at the outset of a task, at intermediate points in a chain, or within a subroutine to select the most appropriate tool. Frameworks like **LangChain**, **LangGraph**, and **Google's Agent Developer Kit (ADK)** provide explicit constructs for managing this conditional logic. LangGraph’s state-based graph architecture is particularly effective for complex scenarios where decisions depend on the cumulative system state.

#### Practical Applications & Use Cases
- **User Intent Interpretation:** Virtual assistants use routing to select modules (e.g., AI tutors choosing curriculum based on performance).
- **Automated Data Pipelines:** Directing incoming items (emails, tickets, API payloads) based on content or metadata (e.g., Urgent vs. Sales lead).
- **High-Level Dispatching:** Research systems use routers to assign tasks to specialized sub-agents (Searcher vs. Summarizer).
- **AI Coding Assistants:** Routing to identify language and intent (Debug, Explain, Translate) before passing snippets to specialized tools.

Ultimately, routing provides the logical arbitration essential for creating functionally diverse, context-aware systems.

#### Hands-On Code Example (LangChain)
This example sets up a "coordinator" that routes user requests to different simulated "sub-agent" handlers based on intent (booking, info, or unclear) using **RunnableBranch**.
...
coordinator_router_prompt = ChatPromptTemplate.from_messages([
   ("system", """Analyze the user's request:
    - If booking flights/hotels, output 'booker'.
    - For general questions, output 'info'.
...
In the ADK paradigm, routing is typically implemented via **Auto-Flow**, where the framework leverages an underlying model to match user intent to a tool handler or sub-agent.
...
# --- Tools ---
def booking_handler(request: str) -> str:
   return f"Booking action for '{request}' simulated."

def info_handler(request: str) -> str:
   return f"Information request for '{request}' handled."
...
booking_agent = Agent(
   ...
   description="Handles flight/hotel booking.", tools=[booking_tool]
)

info_agent = Agent(
   name="Info", model="gemini-2.0-flash",
   description="Provides general info.", tools=[info_tool]
)

coordinator = Agent(
   name="Coordinator", model="gemini-2.0-flash",
   instruction="Analyze request and delegate to Booker or Info agents. Do not answer directly.",
...
async def run_coordinator(runner: InMemoryRunner, request: str):
   ...
   await runner.session_service.create_session(app_name=runner.app_name, user_id="user_1", session_id=session_id)
   for event in runner.run(user_id="user_1", session_id=session_id, new_message=types.Content(role='user', parts=[types.Part(text=request)])):
       if event.is_final_response() and event.content:
           return event.content.text
```

#### Key Takeaways
- **Dynamic Decision Making:** Enables agents to navigate non-linear workflows based on context.
- **Specialization:** Leverages the best sub-process for a specific task.
- **Architectural Flexibility:** Frameworks like LangGraph (state-based) vs. ADK (auto-flow) offer different mature ways to implement this.
...
ttps://www.langchain.com/
2.### Chapter 3: Parallelization

| Feature | Description |
|---|---|
| **What** | Concurrent execution of multiple components (LLM calls, tool usages, or sub-agents) to reduce total execution time. |
| **Why** | Optimizes performance for independent sub-tasks, especially when dealing with high-latency external APIs or databases. |
| **Rule of Thumb** | Use when tasks do not depend on each other's outputs and can be aggregated later (e.g., Multi-source research). |

---

#### Pattern Overview
In previous chapters, we explored **Prompt Chaining** for sequential workflows and **Routing** for dynamic decision-making. While essential, many complex agentic tasks involve multiple sub-tasks that can be executed simultaneously. This is where the **Parallelization** pattern becomes crucial.

Parallelization involves executing multiple components, such as LLM calls, tool usages, or even entire sub-agents, concurrently (see Fig. 1). Instead of waiting for one step to complete before starting the next, parallel execution allows independent tasks to run at the same time, significantly reducing overall latency for tasks that can be decomposed into independent parts.

*Sequential vs. Parallel Research Example:*
- **Sequential:** Search Source A → Summarize A → Search Source B → Summarize B → Synthesize.
- **Parallel:** (Search A & Search B) simultaneously → (Summarize A & Summarize B) simultaneously → Synthesize.

The core idea is to identify parts of the workflow that do not depend on the output of other parts. This is particularly effective when dealing with external services (APIs or databases) that have inherent latency.

#### Implementation & Frameworks
Modern agentic frameworks are designed with asynchronous operations in mind:
- **LangChain (LCEL):** Achieve parallelism by nesting runnables in a dictionary or using the `RunnableParallel` construct.
- **LangGraph:** Define multiple nodes that can be initiated from a single state transition, enabling parallel branches.
- **Google ADK:** Provides robust, native mechanisms for parallel execution of agents, significantly enhancing scalability in multi-agent systems.

#### Practical Applications & Use Cases
1.  **Information Gathering:** Investigating a topic by searching news, pulling stock data, and checking social media simultaneously.
2.  **Data Analysis:** Running sentiment analysis, keyword extraction, and categorization concurrently across a batch of feedback.
3.  **Multi-Tool Interaction:** A travel agent checking flight prices, hotel availability, and local events at the same time.
4.  **Content Component Generation:** Drafting an email subject line, body, and call-to-action button text in parallel.
5.  **Multi-Modal Processing:** Analyzing a social media post's text and image concurrently for integrated insights.
6.  **A/B Testing:** Generating multiple variations of a response simultaneously to select the highest-quality option.

#### Hands-On Code Example (LangChain)
Parallel execution in LangChain is typically handled via `RunnableParallel` (or a dictionary), which LCEL executes concurrently.
...
# --- Independent Chains ---
summarize_chain = (
   ChatPromptTemplate.from_template("Summarize the following topic concisely: {topic}") 
   | llm | StrOutputParser()
)

questions_chain = (
   ChatPromptTemplate.from_template("Generate three interesting questions about: {topic}") 
   | llm | StrOutputParser()
)

terms_chain = (
   ChatPromptTemplate.from_template("Identify 5-10 key terms for: {topic}") 
   | llm | StrOutputParser()
)
...
synthesis_prompt = ChatPromptTemplate.from_template("""
    ...
    Questions: {questions}
    Terms: {key_terms}
    Based on this, synthesize a comprehensive answer for {topic}.
""")
...
#### Hands-On Code Example (Google ADK)
In Google ADK, `ParallelAgent` orchestrates the concurrent execution of multiple sub-agents.
...
# --- Sub-Agents ---
res_1 = LlmAgent(name="EnergyRes", instruction="Research renewable energy.", tools=[google_search], output_key="res_1")
res_2 = LlmAgent(name="EVRes", instruction="Research electric vehicles.", tools=[google_search], output_key="res_2")

# --- Parallel Orchestration ---
parallel_agent = ParallelAgent(name="ParallelResearch", sub_agents=[res_1, res_2])

# --- Synthesis ---
merger_agent = LlmAgent(
    name="SynthesisAgent",
    instruction="Synthesize findings from {res_1} and {res_2}. Ground strictly in these inputs."
)

# --- Sequence ---
root_agent = SequentialAgent(name="Pipeline", sub_agents=[parallel_agent, merger_agent])
```

#### Key Takeaways
- **Efficiency:** Drastically reduces total execution time by leveraging concurrency.
- **Complexity Trade-off:** Parallel architectures increase design, debugging, and logging complexity.
- **Framework Support:** LCEL (`RunnableParallel`) and ADK (`ParallelAgent`) make this advanced pattern accessible.

#### References
1.  [LangChain LCEL Documentation (Parallelism)](https://python.langchain.com/docs/concepts/lcel/)
...
ttps://google.github.io/adk-docs/agents/multi-a### Chapter 4: Reflection

| Feature | Description |
|---|---|
| **What** | A control pattern where an agent evaluates its own work, output, or internal state to iteratively refine its performance. |
| **Why** | Moves beyond single-pass generation to ensure high-quality, accurate, and instruction-compliant results through corrective feedback loops. |
| **Rule of Thumb** | Use when accuracy and output quality are prioritized over latency and cost (e.g., Code generation, Law, or Medical summary). |

---

#### Pattern Overview
While Patterns like Chaining, Routing, and Parallelization enable complex task execution, an agent's initial output may still be suboptimal or inaccurate. The **Reflection** pattern addresses this by introducing a feedback loop for self-correction or third-party critique.

In this model, the agent doesn't just produce output; it examines the result, identifies flaws (factual errors, stylistic issues, missing constraints), and uses those insights to generate a refined version. This process typically involves:
1.  **Execution:** Initial generation of the task output.
2.  **Evaluation/Critique:** Analysis of the result (via LLM-critique or automated rules).
3.  **Reflection/Refinement:** Adjusting the approach or content based on the critique.
4.  **Iteration:** Repeating the cycle until a stopping condition (quality threshold or max turns) is met.

#### The Generator-Critic Model
A highly effective implementation separates the process into two logical roles:
- **The Producer:** Focuses entirely on content generation (e.g., writing code).
- **The Critic:** A specialized evaluator with a distinct persona (e.g., "Senior Security Engineer") designed to find flaws and suggest improvements.

This separation prevents "cognitive bias" where an agent overlooks its own errors. Frameworks like **LangGraph** (stateful loops) and **Google ADK** (Sequential/Loop agents) provide native support for these architectures.

#### Practical Applications & Use Cases
- **Creative Writing:** Drafting a post, critiquing it for tone/clarity, and rewriting for polish.
- **Code Debugging:** Writing a function, running tests, identifying errors, and fixing them.
- **Problem Solving:** Proposing logic steps, evaluating them for contradictions, and backtracking if necessary.
- **Info Synthesis:** Comparing a summary against original source points to ensure no critical data loss.
- **Strategy Planning:** Simulating a plan against constraints and revising it before final execution.

#### Hands-On Code Example (LangChain)
This example demonstrates a multi-step reflection loop to improve a Python function calculation using a "Senior Reviewer" critique persona.
...
def run_reflection_loop():
   task = "Create a robust Python function `calculate_factorial(n)` with error handling and docstrings."
   ...
   for i in range(3):
       ...
       # 1. Generate/Refine
       if i > 0:
           history.append(HumanMessage(content="Refine the code based on the critique."))
       ...
       # 2. Reflect (Critic)
       critic_prompt = [
           SystemMessage(content="Review this Python code. If perfect, say 'CODE_IS_PERFECT'. Otherwise, list critiques."),
           HumanMessage(content=f"Task: {task}\nCode:\n{code}")
       ]
       critique = llm.invoke(critic_prompt).content
       
       if "CODE_IS_PERFECT" in critique:
           print("Status: Code is satisfactory.")
           break
...
#### Hands-On Code Example (Google ADK)
In ADK, reflection is often structured as a `SequentialAgent` where a producer passes its state to a reviewer.
...
generator = LlmAgent(
   name="Writer",
   instruction="Write a paragraph on the subject.",
   output_key="draft"
)

reviewer = LlmAgent(
   name="FactChecker",
   instruction="Read {draft}. Output status (ACCURATE/INACCURATE) and reasoning.",
   output_key="review"
)

# --- Pipeline ---
review_pipeline = SequentialAgent(name="RefinementFlow", sub_agents=[generator, reviewer])
```

#### Key Takeaways
- **Self-Correction:** Enables agents to enhance performance over time without human intervention.
- **Objectivity:** The Producer-Critic model significantly improves accuracy by separating intent from evaluation.
- **Trade-offs:** Higher quality comes at the cost of increased latency, cost (more LLM calls), and context window pressure.

#### References
1.  [Training Models to Self-Correct (RL)](https://arxiv.org/abs/2409.12917)
2.  [LangChain LCEL Documentation](https://python.langchain.com/docs/introduction/)
3.  [LangGraph Documentation](https://www.langchain.com/langgraph)
4.  [Google ADK (Multi-Agent Systems)](https://google.github.io/adk-docs/agents/multi-agents/)
...
4.  Google Agent Developer Kit (ADK) Docu### Chapter 5: Tool Use (Function Calling)

| Feature | Description |
|---|---|
| **What** | A mechanism (often triggered via JSON function calling) that enables LLMs to interact with external APIs, databases, or local code. |
| **Why** | Bridges the gap between internal reasoning and real-world utility, allowing agents to fetch current data or execute deterministic calculations. |
| **Rule of Thumb** | Use when the agent needs access to real-time information or must perform actions that an LLM cannot do alone (e.g., Search, Math, CRUD). |

---

#### Pattern Overview
While Patterns like Chaining and Routing manage internal flows, the **Tool Use** pattern (technically implemented as **Function Calling**) is what grants agents agency in the real world. It allows the model to decide—based on user intent—when to invoke an external service, what arguments to pass, and how to process the result.

The standard execution lifecycle:
1.  **Tool Definition:** Describing a function’s name, purpose, and schema (parameters/types) to the LLM.
2.  **LLM Decision:** The model analyzes the request and determines if a tool is required.
3.  **Call Generation:** The LLM outputs a structured object (usually JSON) containing the tool name and extracted arguments.
4.  **Execution:** The orchestration layer (LangChain, ADK, etc.) intercepts the request and runs the actual code/API.
5.  **Observation:** The result is returned to the LLM.
6.  **Final Processing:** The LLM integrates the "observation" back into its reasoning to provide a final response or decide on the next step.

#### Practical Applications & Use Cases
- **Dynamic Retrieval:** Accessing a weather API, stock market feed, or real-time news.
- **System IR/CRUD:** Querying a database for order status or updating a CRM record.
- **Deterministic Math:** Using a Python interpreter or a specialized calculator to avoid LLM "hallucinations" in arithmetic.
- **Communication:** Triggering automated emails, Slack messages, or SMS alerts.
- **IoT/Hardware Control:** Sending commands to smart home devices or factory sensors.
- **Agent Delegation:** Treating a secondary specialized agent (e.g., a "Security Agent") as a "tool" to be called.

#### Hands-On Code Example (LangChain)
In LangChain, tools are defined using the `@tool` decorator and bound to the LLM.
...
def search_information(query: str) -> str:
    """Provides factual info. Use for capitals, weather, or populations."""
    print(f"--- TOOL CALLED: {query} ---")
    data = {"weather in london": "Cloudy, 15°C.", "capital of france": "Paris."}
    return data.get(query.lower(), f"No specific info for '{query}'.")
...
# --- Execution ---
async def main():
    res = await executor.ainvoke({"input": "What is the capital of France?"})
    print(f"Final Response: {res['output']}")
...
CrewAI agents use tools as capabilities assigned during initialization.
...
def get_stock_price(ticker: str) -> float:
    """Fetches stock prices for AAPL, GOOGL, MSFT."""
    prices = {"AAPL": 178.15, "GOOGL": 1750.30}
    return prices.get(ticker.upper(), 0.0)

analyst = Agent(
    role='Financial Analyst',
    goal='Report stock prices.',
    backstory='Data veteran.',
    tools=[get_stock_price]
)

task = Task(description="Find AAPL price.", expected_output="Price of AAPL.", agent=analyst)
crew = Crew(agents=[analyst], tasks=[task])
result = crew.kickoff()
```

#### Hands-On Code Example (Google ADK)
ADK provides native support for pre-built tools like `google_search` or `BuiltInCodeExecutor`.
...
search_agent = LlmAgent(
    ...
    instruction="Search the web for news.",
    tools=[google_search]
)

# Calculator Agent (Deterministic Execution)
calc_agent = LlmAgent(
    name="MathAgent",
    model="gemini-2.0-flash",
    instruction="Execute Python for math.",
    code_executor=BuiltInCodeExecutor()
)
```

#### Key Takeaways
- **Bridges Reality:** Tool use is the "act" part of "sense-reason-act."
- **Structured Communication:** Reliable tool use depends on clear schema definitions and high-quality descriptions.
- **Vertex AI / Enterprise:** Tools like `VSearchAgent` enable grounding in secure, massive datastores.

#### References
1.  [LangChain Tools Documentation](https://python.langchain.com/docs/concepts/tools/)
2.  [CrewAI Tool Documentation](https://docs.crewai.com/concepts/Tools/)
3.  [Google ADK Built-in Tools](https://google.github.io/adk-docs/tools/built-in-tools/)
...
| Feature | Description |
|---|---|
| **What** | A strategic pattern where a model decomposes a high-level goal into a sequence of discrete, actionable steps before execution. |
| **Why** | Essential for complex, multi-faceted tasks where the "how" isn't predefined and must be discovered through reasoning. |
| **Rule of Thumb** | Use when a request requires interdependent operations or long-running exploration (e.g., Deep Research, Project Planning). |

---

#### Pattern Overview
Intelligent behavior requires more than reactivity; it requires foresight. The **Planning** pattern moves an agent beyond simple "prompt-response" cycles by formulating a roadmap to connect an initial state to a target goal.

Unlike a fixed workflow (where the steps are known), a planning agent autonomously charts its own course. This approach is highly adaptive—if a step fails (e.g., a venue is booked), the agent re-evaluates and reformulates the plan rather than simply failing.

#### Practical Applications & Use Cases
- **Procedural Automation:** Onboarding employees (account creation → training → coordination).
- **Autonomous Navigation:** State-space traversal where a system must optimize for time/energy while avoiding obstacles.
- **Information Synthesis:** Generating a research report (gathering → summarization → structuring → refinement).
- **Strategic Analysis:** Competitive research or market mapping requiring iterative discovery.

#### Hands-On Code Example (CrewAI)
CrewAI uses agents to first formulate a plan and then execute it sequentially.
...
planner = Agent(
   ...
   goal='Create a bulleted plan and write a 200-word summary on RL in AI.',
   backstory='An expert in decomposing complex AI concepts into actionable roadmaps.',
   llm=llm
)

# --- Task ---
research_task = Task(
   description="1. Create a bullet-point plan for 'RL in AI'. 2. Write the summary based on the plan.",
   expected_output="A report with ### Plan and ### Summary sections.",
   agent=planner
)

# --- Crew ---
crew = Crew(agents=[planner], tasks=[research_task], process=Process.sequential)
result = crew.kickoff()
print(result)
```

#### Industrial Spotlight: Deep Research
Deep Research represents the pinnacle of the Planning pattern, shifting from single queries to long-running, asynchronous investigations.

- **Google DeepResearch:** A multi-step pipeline that deconstructs a prompt into a research plan, iteratively queries Google Search, identifies knowledge gaps, and synthesizes a multi-page report with citations.
- **Key Architectural Components:** Asynchrony (resilient to failure), iterative search-and-analysis loops, and interactive report synthesis (including audio overviews).

#### Hands-On Code Example (OpenAI Deep Research API)
The OpenAI API provides direct access to autonomous reasoning models (like `o3-deep-research`) that perform web exploration and citation-rich reporting.
...
response = client.responses.create(
 ...
 input=[
   {"role": "developer", "content": "You are a professional researcher. Focus on data-rich insights and citations."},
   {"role": "user", "content": "Research the economic impact of semaglutide on global healthcare."}
 ],
...
for step in response.output:
    if step.type == "reasoning":
        print(f"Internal Logic: {step.summary}")
    elif step.type == "web_search_call":
        print(f"Query Executed: {step.action['query']}")
```

#### Key Takeaways
- **Discovery vs. Execution:** Use planning when the solution path must be discovered; use fixed workflows when the path is known.
- **Transparency:** Modern APIs (like OpenAI's) expose intermediate reasoning and tool calls for debugging and trust.
- **Adaptability:** The hallmark of a great planner is the ability to pivot when constraints change during execution.

#### References
1.  [Google DeepResearch (Gemini)](https://gemini.google.com)
2.  [OpenAI Deep Research Introduction](https://openai.com/index/introducing-deep-research/)
3.  [Perplexity Deep Research](https://www.perplexity.ai/hub/blog/introducing-perplexity-deep-research)
...
| Feature | Description |
|---|---|
| **What** | A distributed architecture where multiple specialized agents cooperate to solve complex, multi-domain tasks. |
| **Why** | Overcomes the limitations of monolithic agents by leveraging task decomposition, role specialization, and parallelization. |
| **Rule of Thumb** | Use when a task requires diverse expertise (e.g., Researcher + Writer + Editor) or benefits from robustness via redundancy. |

---

#### Pattern Overview
While monolithic agents are effective for simple tasks, complex objectives require a "team" approach. **Multi-Agent Collaboration** breaks down a high-level goal into discrete sub-problems assigned to specialized agents with tailored tools, data access, or reasoning capabilities.

#### Collaboration Models
Agents can interact through several structural paradigms:
- **Sequential Handoffs:** A linear pipeline where Agent A’s output informs Agent B.
- **Parallel Processing:** Concurrent task execution combined in a final synthesis step.
- **Debate & Consensus:** Multiple agents with varying perspectives discuss and arrive at a single, high-quality decision.
- **Hierarchical Structure:** A "Manager" agent delegates sub-tasks to "Worker" agents and synthesizes results.
- **Critic-Reviewer:** A producer agent creates a draft, while a critic agent assesses it for compliance, security, or quality.

#### Practical Applications
- **Software Development:** Requirements Analyst → Developer → Tester → Documenter.
- **Financial Analysis:** Stock Data Fetcher → Sentiment Analyzer → Technical Analyst → Portfolio Strategist.
- **Creative Campaigns:** Market Researcher → Copywriter → Graphic Designer → Social Media Scheduler.
- **Network Remediation:** Triage Agent → Root Cause Specialist → Remediation Executor.

#### Communication Architectures
As shown in industrial frameworks, communication can be:
- **Network (Decentralized):** Peer-to-peer sharing, high resilience, but high coordination overhead.
- **Supervisor (Centralized):** A hub for task allocation and conflict resolution.
- **Hierarchical (Multi-Layer):** Supervisors overseeing other supervisors for massive-scale decomposition.

#### Hands-On Code Example (CrewAI)
This example demonstrates a sequential collaboration between a Researcher and a Writer using Gemini 2.0.
...
researcher = Agent(
    ...
    backstory='Expert in trend synthesis and strategic data identification.',
    verbose=True
)
...
writer = Agent(
    ...
    backstory='Skilled at translating complex technical data into accessible narratives.',
    verbose=True
)

# --- Tasks ---
task_research = Task(
    description="Research top 3 emerging trends in AI for 2025.",
    expected_output="A detailed summary of 3 AI trends with impact analysis.",
    agent=researcher
)
...
#### Hands-On Code Example (Google ADK: Hierarchical)
The Google ADK simplifies parent-child relationships for delegation.
...
print("Agent hierarchy for 'Coordinator -> Greeter' established.")
```

#### Hands-On Code Example (Google ADK: Loop/Iterative)
The `LoopAgent` enables repetitive workflows until a condition is met.

```python
from google.adk.agents import LoopAgent, LlmAgent, BaseAgent

# --- Condition Checker Artist ---
...
#### Key Takeaways
- **Modularity:** Failure of one specialized agent doesn't necessarily crash the entire system.
- **Synergy:** The collective effort of role-specific agents often yields higher quality than one generalist agent.
- **Communication Protocol:** Clear data-passing (via context or session state) is the "glue" of multi-agent success.

#### References
1. [Multi-Agent Collaboration Mechanisms: A Survey of LLMs](https://arxiv.org/abs/2501.06322)
2. [Multi-Agent Systems: The Power of Collaboration](https://aravindakumar.medium.com/introducing-multi-agent-frameworks-the-power-of-collaboration-e9db31bba1b6)
